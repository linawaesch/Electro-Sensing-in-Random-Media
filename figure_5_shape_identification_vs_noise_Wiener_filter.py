...
def load_recovered_G(baseline_npz: str = "order_out_baseline.npz"):
    ...
    We prefer 'G_rec', whose columns are aligned with grid_points.
    """
...
def wiener2d_complex(image: np.ndarray, kernel_size: int = 3, noise_power: float = None) -> np.ndarray:
    """
    Simple local Wiener filter for a 2D complex-valued image.

    Parameters
    ----------
    image : 2D ndarray (complex or real)
        Input image on the (z,x) grid.
    kernel_size : int
        Size of the square neighbourhood window (must be odd).
    noise_power : float or None
        Assumed noise variance. If None, it is estimated from the
        smallest 10% of local variances.
    ...
    if img.ndim != 2:
        raise ValueError(f"wiener2d_complex expects a 2D array, got shape {img.shape}")
    ...
    nz, nx = img.shape
    local_mean = np.zeros_like(img, dtype=np.complex128)
    local_var = np.zeros_like(img, dtype=float)

    # Compute local mean and variance in each k x k window
    for i in range(nz):
        for j in range(nx):
            window = padded[i:i + k, j:j + k]
            mu = window.mean()
            local_mean[i, j] = mu
            local_var[i, j] = np.mean(np.abs(window - mu) ** 2)

    # Estimate noise variance if not provided: use mean of smallest 10% of local variances
    if noise_power is None:
        flat = np.sort(local_var.ravel())
        m0 = max(1, int(0.1 * flat.size))
        noise_power = float(flat[:m0].mean())

    # Avoid division by zero
    var_clipped = np.maximum(local_var, 1e-12)

    # Wiener gain factor
    gain = 1.0 - noise_power / var_clipped
    gain = np.clip(gain, 0.0, 1.0)

    out = local_mean + gain * (img - local_mean)
    return out


def wiener_filter_G_stack(G_stack: np.ndarray, meta, kernel_size: int = 3, noise_power: float = None) -> np.ndarray:
    """
    Apply a simple 2D local Wiener filter to each (frequency,receiver) slice
    of a Green's matrix stack over the (z,x) grid.

    Parameters
    ----------
    G_stack : ndarray
        Array of shape (Nf*Nr, K) or a shape accepted by coerce_G_stack.
    meta : dict
        Meta information containing Nx, Nz, Nf, Nr.
    kernel_size : int
        Window size for the local Wiener filter.
    noise_power : float or None
        Noise variance used by the Wiener filter. If None, estimated
        separately for each slice.

    Returns
    -------
    G_filtered : ndarray
        Wiener-filtered Green's matrix stack with shape (Nf*Nr, K).
    """
    Nx = int(meta["Nx"])
    Nz = int(meta["Nz"])
    Nf = int(meta["Nf"])
    ...
    # Ensure we are in (Nf*Nr, K) shape
    G_stack = coerce_G_stack(G_stack, meta)

    G_filt = np.empty_like(G_stack)
    for row in range(Nf * Nr):
        field = G_stack[row, :].reshape(Nz, Nx)
        field_filt = wiener2d_complex(field, kernel_size=kernel_size, noise_power=noise_power)
        G_filt[row, :] = field_filt.reshape(K)
...
def run_shape_identification_experiment(
    ...
    # Apply Wiener filter to the recovered G to stabilise its gradients
    print("Applying Wiener filter to recovered G_rec_stack for gradient stabilisation...")
    G_rec_stack = wiener_filter_G_stack(G_rec_stack, meta, kernel_size=3, noise_power=None)
    print("Done Wiener filtering of recovered G.\n")
    ...
    plt.plot(noise_levels, acc_rec  * 100.0, "-s", label="Using G_rec (Wiener filtered)")
